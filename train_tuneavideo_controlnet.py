...
from tuneavideo.data.dataset import TuneAVideoControlNetDataset
from tuneavideo.pipelines.pipeline_tuneavideo_controlnet import (
    TuneAVideoControlNetPipeline,
)
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    controlnet = UNet2DConditionModel.from_pretrained(
        pretrained_model_path, subfolder="controlnet"
    )
    ...
    # Freeze vae, text_encoder and controlnet
    ...
    if enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            ...
            controlnet.enable_xformers_memory_efficient_attention()  # is this working?
        else:
            raise ValueError(
    ...
    train_dataset = TuneAVideoControlNetDataset(**train_data)
    ...
    validation_pipeline = TuneAVideoControlNetPipeline(
        vae=vae,
    ...
    # Move text_encode, vae and controlnet to gpu and cast to weight_dtype
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                pixel_values_hint = batch["pixel_values_hint"].to(weight_dtype)
                video_length = pixel_values.shape[1]
                pixel_values = rearrange(pixel_values, "b f c h w -> (b f) c h w")
                controlnet_hint = rearrange(
                    pixel_values_hint, "b f c h w -> (b f) c h w"
                )  # idk if it works with b > 1
                ...
                model_pred = unet(
                    noisy_latents, timesteps, encoder_hidden_states, control=cat_control
                ).sample
    ...
    if accelerator.is_main_process:
        ...
        pipeline = TuneAVideoControlNetPipeline.from_pretrained(
